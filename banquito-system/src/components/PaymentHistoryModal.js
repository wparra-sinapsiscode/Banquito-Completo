...
const PaymentHistoryModal = ({ loan, member, onClose }) => {
  ...
  const getStatistics = () => {
    ...
    // Calcular total pagado correctamente (parseFloat para evitar concatenación)
    const frontendTotal = payments.reduce((sum, p) => sum + (parseFloat(p.amount) || 0), 0);
    const backendTotal = backendPayments.reduce((sum, p) => sum + (parseFloat(p.amount) || 0), 0);
    const totalPaid = Math.max(frontendTotal, backendTotal);
    
    // Calcular pagos realizados y restantes
    const totalPayments = Math.max(payments.length, backendPayments.length);
    const totalWeeks = loan.totalWeeks || loan.installments || 0;
    const remainingPayments = Math.max(0, totalWeeks - totalPayments);
    
    // Calcular progreso basado en lo pagado vs total con intereses
    const totalWithInterest = (loan.weeklyPayment || loan.monthlyPayment || 0) * totalWeeks;
    const progressPercentage = totalWithInterest > 0 ? (totalPaid / totalWithInterest) * 100 : 0;
